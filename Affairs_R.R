# Load the Dataset
affairs <- read.csv(file.choose()) # Choose the claimants Data set
affairs$naffairs <- ifelse(affairs$naffairs > 0.5, 1, 0)
sum(is.na(affairs))

# Omitting NA values from the Data 
affairs1 <- na.omit(affairs) # na.omit => will omit the rows which has atleast 1 NA value
dim(affairs1)
...
affairs1 <- affairs1[ , -1] # Removing the first column which is is an Index
